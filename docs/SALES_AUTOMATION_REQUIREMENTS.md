...
| **収集データ** | **必須**：企業名、WebサイトURL<br>**オプション**：<br>・問い合わせフォームURL、業種、従業員数、資本金、設立年、所在地、連絡先（全業種で有効）<br>・**カスタム項目1〜3**：検索対象に応じてClaude Codeが自動的に収集内容を判断<br>　例：IT企業 → 技術スタック、開発実績、エンジニア数<br>　例：製造業 → 主要製品、工場所在地、ISO認証<br>　例：資金調達企業 → 調達ラウンド、調達額、調達日 |
| **並列実行** | **15コンテナ並列**（最大限活用） |
